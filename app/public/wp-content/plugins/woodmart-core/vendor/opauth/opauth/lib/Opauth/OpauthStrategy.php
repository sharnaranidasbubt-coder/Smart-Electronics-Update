...
class OpauthStrategy {
	...
	protected function expects($key, $not = null) {
		if (!array_key_exists($key, $this->strategy)) {
			wp_trigger_error( __METHOD__, $this->name." config parameter for \"$key\" expected.", E_USER_ERROR);
			exit();
		}
	...
	public static function httpRequest($url, $options = null, &$responseHeaders = null) {
		if ( apply_filters( 'woodmart_alt_auth_method', false ) || ( function_exists( 'woodmart_get_opt' ) && woodmart_get_opt( 'alt_auth_method' ) ) ) {
			if ( !function_exists( 'curl_init' ) ) {
	            wp_trigger_error( __METHOD__,  'Curl not supported, try using other http_client_method such as file', E_USER_ERROR );
	        }
